...
# This program estimates the reduced-form regressions for the elasticity study. 
...
# ---- Parametric plot of reduced form ------------------------------------------------------------
...
delinq_df = elast_df %>%
  ...
  summarise(
    mean =  100 * mean(delinq_90_24, na.rm = TRUE),
  ) %>%
  ungroup() %>%
  arrange(
    dist
...
gg1 = srd_plot(
  ...
  ylab = "Default rate * 100"
)

# Export plot
cairo_pdf(
  filename = "../output/figures/elast/param_reduced_form_24mo.pdf",
  onefile = TRUE
)
print(gg1)
dev.off()
 
# ---- Parametric plots of each reduced form ------------------------------------------------------
...
# Export plot
cairo_pdf(
  filename = "../output/figures/elast/param_reduced_form_24mo_lender.pdf",
  onefile = TRUE
)
print(gg2)
dev.off()
...
write_csv(
  base_df,
  path = "../output/tables/elast/base_param_reduced.csv"
)
...
write_csv(
  robust_df,
  path = "../output/tables/elast/robust_param_reduced_24.csv"
)
